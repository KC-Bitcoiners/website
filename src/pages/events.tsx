...
import { eventsConfig, getMeetupUrl } from "@/config";
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="text-center mb-12">
        <h1 className="text-5xl font-bold mb-6 font-archivo-black">{eventsConfig.title}</h1>
        ...
        {upcomingEvents.length > 0 ? (
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
            <p className="text-gray-600 text-lg mb-4">
              {eventsConfig.sections.upcoming.emptyMessage}
            </p>
            <p className="text-gray-500">
              {eventsConfig.sections.upcoming.emptySubtext}{" "}
              <a
                href={getMeetupUrl()}
                target="_blank"
      ...
      {pastEvents.length > 0 && (
        <section>
          ...
          <div className="space-y-8 opacity-75">
            {pastEvents.slice(0, eventsConfig.sections.past.showCount).map((event) => (
              <EventCard
                key={event.id}
                date={formatDate(event.dateTime)}
                title={event.title}
          ...
          {pastEvents.length > eventsConfig.sections.past.showCount && (
            <div className="text-center mt-8">
              <p className="text-gray-500">
                Showing the {eventsConfig.sections.past.showCount} most recent past events.{" "}
                <a
      ...
      <section className="mt-16 bg-gradient-to-r from-gray-50 to-orange-50 border border-gray-200 rounded-lg p-8 text-center">
        <h3 className="text-2xl font-bold mb-4 font-archivo-black">
          {eventsConfig.sections.callToAction.title}
        </h3>
        <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
          {eventsConfig.sections.callToAction.description}
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          >
            {eventsConfig.sections.callToAction.buttonText}
          </a>
