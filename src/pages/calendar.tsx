...
import { fetchMeetupEvents, getVenueAddress, MeetupGroup } from '../utils/meetup';
import { fetchNostrCalendarEvents, convertNostrEventToCalendar, publishNostrEvent } from '../utils/nostrEvents';
...
export default function CalendarPage({ meetupGroup, meetupError }: InferGetStaticPropsType<typeof getStaticProps>) {
      ...
      try {
        ...
        if (meetupGroup) {
          ...
          meetupEvents = meetupGroup.events.edges.map((edge) => {
            ...
            const endTime = event.endTime ? Math.floor(new Date(event.endTime).getTime() / 1000) : startTime + 3600; // Default 1 hour duration
            
            console.log(`🎯 Processing meetup event: ${event.title} at ${new Date(startTime * 1000).toLocaleString()}`);
          ...
          console.log(`✅ Successfully processed ${meetupEvents.length} meetup events`);
        } else {
          console.log('⚠️ No meetup group data available');
        }

        // Load local and meetup events immediately
        console.log(`🚀 Displaying immediate events: ${localEvents.length} local + ${meetupEvents.length} meetup`);
        console.log('🔍 Meetup events before sorting:', meetupEvents.map(e => ({ 
          id: e.id, 
          title: e.title, 
          start: e.start,
          kind: e.kind,
        ...
        console.log('🔍 All immediate events after sorting:', immediateEvents.map(e => ({ 
          id: e.id, 
          title: e.title, 
          start: e.start,
          kind: e.kind,
        ...
        console.log('🔍 Upcoming events:', upcoming.map(e => ({ id: e.id, title: e.title, start: e.start, kind: e.kind })));
        console.log('🔍 Past events:', past.map(e => ({ id: e.id, title: e.title, start: e.start, kind: e.kind })));
  ...
  useEffect(() => {
    const loadNostrEvents = async () => {
      ...
      setEvents(prevEvents => {
        console.log(`➕ Adding ${nostrEvents.length} nostr events to existing ${prevEvents.length} events`);
    ...
    try {
      ...
      if (result.success) {
        ...
        setEvents(prevEvents => {
          ...
          if (exists) {
            console.log('⚠️ Event already exists, skipping duplicate:', newEvent.id);
            return prevEvents;
          }
          ...
          const hasLocalDuplicate = existingEvents.some(e => 
            e.id === newEvent.id || 
            (e.dTag === newEvent.dTag && e.pubkey === newEvent.pubkey)
          );
        ...
        setTimeout(async () => {
          console.log('🔄 Fetching newly published event from relay to verify it was published...');
          try {
            console.log('📡 Calling fetchNostrCalendarEvents() to get latest events from relay...');
            const nostrCalendarEvents = await fetchNostrCalendarEvents();
            console.log(`📨 Received ${nostrCalendarEvents.length} events from relay`);
            ...
            setEvents(prevEvents => {
              const allEvents = sortEventsByTime([...prevEvents, ...nostrEvents]);
              console.log(`📅 Updated total events: ${allEvents.length} (was ${prevEvents.length})`);
              return allEvents;
            });
            ...
            console.warn('⚠️ Event was published successfully, but failed to refresh from relay');
          }
        ...
        if (errorMsg.includes('timeout') || errorMsg.includes('Timeout')) {
          alert('🕐 Publishing timed out. The relays may be busy or experiencing issues. Please try again in a moment.\n\nError: ' + errorMsg);
        } else if (errorMsg.includes('connection') || errorMsg.includes('Connection')) {
          alert('🔌 Connection to relays failed. Please check your internet connection and try again.\n\nError: ' + errorMsg);
        } else if (errorMsg.includes('Failed to publish to any relay')) {
          alert('📡 Unable to publish to any relay. The event data may be invalid or relays may be experiencing issues. Please try again.\n\nError: ' + errorMsg);
        } else {
  ...
  const getEventColor = (event: CalendarEvent): string => {
    ...
    return colorIndex >= 0 ? colors[colorIndex % colors.length] : 'bg-gray-50 border-gray-200'; // Default fallback
  };
          ...
          <div className="w-24 flex-shrink-0">
            <div className="sticky top-24 space-y-2">
              <div className="bg-white border border-gray-200 rounded-lg p-2 text-center shadow-sm">
                <div className="text-lg font-bold text-bitcoin-orange mb-1">{events.length}</div>
                <div className="text-xs text-gray-600">Total</div>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-2 text-center shadow-sm">
                <div className="text-lg font-bold text-green-600 mb-1">{upcomingEvents.length}</div>
                <div className="text-xs text-gray-600">Upcoming</div>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-2 text-center shadow-sm">
                <div className="text-lg font-bold text-gray-600 mb-1">{pastEvents.length}</div>
            ...
            {viewMode !== 'list' && isLoadingNostrEvents && (
              <div className="absolute top-4 left-0 right-0 z-50 flex justify-center">
                <div className="flex flex-col items-center gap-3 px-6 py-3 bg-white bg-opacity-95 rounded-lg shadow-lg backdrop-blur-sm">
                  ...
                  />
                  <p className="text-purple-600 font-medium">Loading events from Nostr...</p>
                </div>
              </div>
            )}
                ...
                {isLoadingNostrEvents && (
                  <div className="bitcoin-shaka-loading-overlay mb-8">
                    <div className="bitcoin-shaka-container">
                      ...
                      />
                      <p className="text-purple-600 font-medium mt-4 text-center">Loading events from Nostr...</p>
                    </div>
                  </div>
                )}
            ...
            <div className="max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <EventForm
                initialData={editingEvent ? {
                  ...
                  locations: editingEvent.locations || (editingEvent.location ? [editingEvent.location] : []),
                  startDate: editingEvent.kind === 31922 ? editingEvent.start || '' : 
                    editingEvent.start ? new Date(parseInt(editingEvent.start) * 1000).toISOString().split('T')[0] : '',
                  endDate: editingEvent.kind === 31922 ? editingEvent.end || '' :
                    editingEvent.end ? new Date(parseInt(editingEvent.end) * 1000).toISOString().split('T')[0] : '',
                  startTime: editingEvent.kind === 31923 ? 
                    editingEvent.start ? new Date(parseInt(editingEvent.start) * 1000).toTimeString().slice(0, 5) : '' : '',
                  endTime: editingEvent.kind === 31923 ?
                    editingEvent.end ? new Date(parseInt(editingEvent.end) * 1000).toTimeString().slice(0, 5) : '' : '',
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full relative">
              ...
              <div className="text-center pr-8">
                ...
                <h3 className="text-xl font-bold font-archivo-black mb-2">Event Published Successfully!</h3>
                <p className="text-gray-600 mb-4">Your event has been published to the Nostr network.</p>
                
                <div className="bg-gray-50 rounded-lg p-3 mb-4">
                  <p className="text-sm font-medium text-gray-700 mb-1">Event ID:</p>
                  <p className="text-xs text-gray-600 break-all font-mono">{successMessage.eventId}</p>
                  <p className="text-sm font-medium text-gray-700 mb-2 mt-3">View Event:</p>
                  <a 
