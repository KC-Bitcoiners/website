...
function extractZapAmount(event: any): number {
  ...
  if (bolt11Tag?.[1]) {
    ...
    // Fallback: amount + unit in invoice string (e.g. "lnbc1u" = 100k msats)
    try {
      ...
      if (invoice.includes("lnbc")) {
        ...
        if (m) {
          ...
          switch (m[2]) {
            ...
            default: return Math.floor(amt * 100000000);
          }
        }
      }
    } catch { /* ignore */ }
...
export function fetchZapTotal(eventId: string): Promise<number> {
  return new Promise((resolve) => {
    let settled = false;
    let total = 0;
    const seen = new Set<string>(); // deduplicate across relays
