...
export class NostrUtils {
  ...
  static createCoordinate(kind: number, pubkey: string, identifier: string): string {
    return `${kind}:${pubkey}:${identifier}`;
  }

  // Parse a coordinate string
  static parseCoordinate(coordinate: string): { kind: number; pubkey: string; identifier: string } | null {
  ...
  static getTagValue(event: NostrEvent, tagName: string, index: number = 0): string | undefined {
  ...
  static filterByTimeRange(events: NostrEvent[], startTime?: number, endTime?: number): NostrEvent[] {
  ...
  static deduplicateReplaceableEvents(events: NostrEvent[]): NostrEvent[] {
    ...
    events.forEach(event => {
      ...
      const coordinate = this.createCoordinate(event.kind, event.pubkey, identifier);
  ...
  static createTextNote(content: string, privateKey: string): Partial<NostrEvent> {
    return {
      kind: 1,
      content,
      tags: [],
