...
export const DISPLAY_TYPE_CONFIG: Record<
  DisplayType,
...
export const ALL_DISPLAY_TYPES: DisplayType[] = [
  "youtube",
  "podcast",
  "podcast-episode",
  "link",
    ...
    pool
      ...
      .subscribe({
        ...
        complete: () => {
          ...
          const pinboards: Pinboard[] = deduped
            .map(parsePinboardEvent)
            .filter(Boolean) as Pinboard[];
...
export async function publishPinboard(
  signedEvent: Record<string, unknown>,
): Promise<boolean> {
  try {
    const responses = await pool.publish(nostrRelays, signedEvent as any);
...
function parsePinboardEvent(event: any): Pinboard | null {
  ...
  const title =
    event.tags?.find((t: string[]) => t[0] === "title")?.[1] || dTag;
  const description =
  ...
  const tags =
    event.tags
      ...
      .map((t: string[]) => t[1]) || [];
  const collaborative =
    event.tags?.some((t: string[]) => t[0] === "collaborative") || false;
...
function parsePinEvent(event: any): Pin | null {
  ...
  const aTag = event.tags?.find(
    (t: string[]) => t[0] === "a" && !t[1]?.startsWith("30067:"),
  );
