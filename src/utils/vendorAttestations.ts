...
export function generateVendorNaddr(kind: number, pubkey: string, identifier: string): string {
  try {
    ...
    console.log(`🔧 Cleaned pubkey for vendor naddr: ${cleanPubkey} (original: ${pubkey})`);
    ...
    try {
      ...
      console.error('Fallback vendor naddr encoding also failed:', fallbackError);
...
export async function fetchVendorAttestations(): Promise<VendorAttestation[]> {
  ...
  console.log(`🔌 Fetching vendor attestations from primary relay: ${primaryRelay}`);
  let primaryEvents: VendorAttestation[] = [];
  
  try {
    const ws = new WebSocket(primaryRelay);
    
    const eventsPromise = new Promise<VendorAttestation[]>((resolve, reject) => {
      ...
      ws.onopen = () => {
        console.log(`✅ Connected to ${primaryRelay} for vendor attestations`);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          console.log(`📨 Vendor attestation message from ${primaryRelay}:`, message);

          if (message[0] === 'EVENT') {
            const [type, subscriptionId, nostrEvent] = message;
            
            if (subscriptionId === 'vendor-attestations-sub') {
              console.log(`🎯 Found vendor attestation from ${primaryRelay}:`, nostrEvent);
              ...
              try {
                const vendorData = JSON.parse(nostrEvent.content) as VendorData;
                vendorAttestation.vendorData = vendorData;
              } catch (parseError) {
                console.warn('Failed to parse vendor data content:', parseError);
              }

              relayEvents.push(vendorAttestation);
            }
          } else if (message[0] === 'EOSE') {
            console.log(`📭 End of stored vendor attestations from ${primaryRelay}`);
            clearTimeout(timeout);
            ws.close();
            resolve(relayEvents);
          }
        } catch (error) {
          console.error(`💥 Error parsing vendor attestation message from ${primaryRelay}:`, error);
        }
      };
      ...
      ws.onclose = () => {
        console.log(`🔌 Closed vendor attestation connection to ${primaryRelay}`);
        clearTimeout(timeout);
        resolve(relayEvents);
      };
    });
    ...
    console.log(`📊 Got ${primaryEvents.length} vendor attestations from primary relay`);
  } catch (error) {
    console.warn(`⚠️ Failed to fetch vendor attestations from primary relay ${primaryRelay}:`, error);
  }

  // Add primary events to allEvents
  for (const event of primaryEvents) {
    allEvents.push(event);
    console.log(`➕ Added primary vendor attestation:`, event.vendorData?.name || 'No name');
  }
  ...
  console.log(`🔍 Existing vendor naddrs from primary relay: ${existingNaddrs.size}`);

  // Now fetch from backup relays and deduplicate
  for (const relayUrl of backupRelays) {
    console.log(`🔌 Fetching vendor attestations from backup relay: ${relayUrl}...`);
    ...
    try {
      ...
      const eventsPromise = new Promise<VendorAttestation[]>((resolve, reject) => {
        ...
        ws.onmessage = (event) => {
          try {
            const message = JSON.parse(event.data);
            console.log(`📨 Vendor attestation message from ${relayUrl}:`, message);

            if (message[0] === 'EVENT') {
              const [type, subscriptionId, nostrEvent] = message;
              
              if (subscriptionId === 'vendor-attestations-sub') {
                console.log(`🎯 Found vendor attestation from ${relayUrl}:`, nostrEvent);
              ...
              console.log(`📭 End of stored vendor attestations from ${relayUrl}`);
              clearTimeout(timeout);
              ws.close();
              resolve(relayEvents);
            }
          } catch (error) {
            console.error(`💥 Error parsing vendor attestation message from ${relayUrl}:`, error);
          }
        };
        ...
        ws.onclose = () => {
          console.log(`🔌 Closed vendor attestation connection to ${relayUrl}`);
          clearTimeout(timeout);
          resolve(relayEvents);
        };
      });
      ...
      for (const event of relayEvents) {
        if (event.dTag) {
          ...
          if (!existingNaddrs.has(naddr)) {
            allEvents.push(event);
            existingNaddrs.add(naddr);
            console.log(`➕ Added backup vendor attestation from ${relayUrl}:`, event.vendorData?.name || 'No name');
          } else {
            console.log(`🔁 Duplicate vendor attestation skipped by naddr:`, event.vendorData?.name || 'No name');
          }
        } else {
          ...
          if (!exists) {
            allEvents.push(event);
            console.log(`➕ Added backup vendor attestation (no dTag) from ${relayUrl}:`, event.vendorData?.name || 'No name');
          } else {
            console.log(`🔁 Duplicate vendor attestation skipped (no dTag):`, event.vendorData?.name || 'No name');
          }
        }
      }
      
    } catch (error) {
      console.warn(`⚠️ Failed to fetch vendor attestations from ${relayUrl}:`, error);
    }
  }

  console.log(`📊 Total vendor attestations fetched: ${allEvents.length}`);
  console.log(`🏪 Vendor attestations summary:`, allEvents.map(e => ({ 
    id: e.id, 
    kind: e.kind, 
...
export function convertVendorAttestationToShopItem(attestation: VendorAttestation): ShopItem | null {
